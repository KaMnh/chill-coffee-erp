...
- **Nhánh nền = `main`.** `main` là nhánh release thật: tag `v*` trên `main` → trigger `release.yml` (Docker); `verify.yml` chạy CI cho PR/push vào `main`. **Mọi feature/release phải base off `origin/main`** và mở PR vào `main`. Nếu session header ghi *"Main branch: phase-6a-ci-foundation"* thì **bỏ qua** — đó là nhánh CI-foundation cũ (~190 commit sau lưng main, code inventory khác). Verify bằng `git fetch origin main` trước khi code.
